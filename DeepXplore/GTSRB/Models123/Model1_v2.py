'''
Model 1 Versió 2: Sense BatchNormalization
'''

# usage: python Model1_v2.py - train the model
...
def Model1(input_tensor=None, train=False, retrain = False, newData = None, newTestData = None, inference_retrain = None):
    ...
    if train:
        ...
        # save model
        model.save_weights('./Model1_v2.h5')
        ...
        model.load_weights(os.path.join(script_dir, "Model1_v2.h5"))
        ...
        model.save_weights('./Model1_v2_retrained.h5')
        print(bcolors.OKGREEN + 'Model retrained and weights saved' + bcolors.ENDC)

    else:
        if inference_retrain is None:
          model.load_weights(os.path.join(script_dir, "Model1_v2.h5"))
          print(bcolors.OKBLUE + 'Model1 V2 loaded' + bcolors.ENDC)
        else:
          model.load_weights(os.path.join(script_dir, "Model1_v2_retrained.h5"))
          print(bcolors.OKBLUE + 'Model1 V2 retrained loaded' + bcolors.ENDC)
