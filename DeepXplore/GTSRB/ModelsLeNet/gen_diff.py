...
parser.add_argument('-occl_size', '--occlusion_size', help="occlusion size (e.g., '10,10')", default="10,10", type=str)
...
for i in xrange(args.seeds):
    # scipy.misc.imsave('./prova/' + 'orig' + '_' + str(i) + '.png', random.choice(x_test))
    rand_index = random.randint(0, len(x_test) - 1)
    iimmgg = x_test[rand_index]
    iimmgg_path = test_df['Path'].iloc[rand_index]
    ...
    if not label1 == label2 == label3:
        print(bcolors.OKGREEN + 'input already causes different outputs: {}, {}, {}'.format(decode_label(label1),
                                                                                            decode_label(label2),
        ...
        scipy.misc.imsave('./generated_inputs/' + 'already_differ_' + decode_label(label1) + '_' + decode_label(label2) + '_' + decode_label(label3) + '_' + str(i) + '.png', gen_img_deprocessed)

        print('./generated_inputs/' + 'already_differ_' + decode_label(label1) + '_' + decode_label(label2) + '_' + decode_label(label3) + '_' + str(i) + '.png')
    ...
    for iters in xrange(args.grad_iterations):
        ...
        if not label1 == label2 == label3:
            ...
            print(bcolors.OKBLUE + 'Models predictions before: Model 1 -> {} ({}), Model 2 -> {} ({}), Model 3 -> {} ({})'.format(
                    decode_label(np.argmax(pred1_before[0])), np.max(pred1_before[0]),
                    decode_label(np.argmax(pred2_before[0])), np.max(pred2_before[0]),
                    decode_label(np.argmax(pred3_before[0])), np.max(pred3_before[0])
                ) + bcolors.ENDC)
            ...
            print(bcolors.OKBLUE + 'Models predictions after: '
                ...
                'Model 3 -> {} (Conf: {:.4f}), (Original predicion conf: {:.4f})'.format(
                    decode_label(label1), np.max(pred1[0]), pred1[0][orig_label],  
                    decode_label(label2), np.max(pred2[0]), pred2[0][orig_label],  
                    decode_label(label3), np.max(pred3[0]), pred3[0][orig_label]  
                ) + bcolors.ENDC)
            ...
            # save the result to disk
            scipy.misc.imsave(
                './generated_inputs/' + args.transformation + '_' + decode_label(label1) + '_' + decode_label(label2) + '_' + decode_label(label3) + '_' + str(i) + '.png', gen_img_deprocessed)
            scipy.misc.imsave(
                './generated_inputs/' + args.transformation + '_' + decode_label(label1) + '_' + decode_label(label2) + '_' + decode_label(label3) + '_' + str(i) + '_orig.png', orig_img_deprocessed)
            
            print('./generated_inputs/' + args.transformation + '_' + decode_label(label1) + '_' + decode_label(label2) + '_' + decode_label(label3) + '_' + str(i) + '.png')
