...
for i in xrange(args.seeds):
    ...
    if not label1 == label2 == label3:
        ...
        imsave('./generated_inputs/' + 'already_differ_' + str(label1) + '_' + str(
            label2) + '_' + str(label3) + '_' + str(i) + '.png', gen_img_deprocessed)

        print('./generated_inputs/' + 'already_differ_' + str(label1) + '_' + str(label2) + '_' + str(label3) + '_' + str(i) + '.png')
    ...
    for iters in xrange(args.grad_iterations):
        ...
        if not predictions1 == predictions2 == predictions3:
            print(bcolors.OKBLUE + 'Models predictions before: Model 1 -> {} ({}), Model 2 -> {} ({}), Model 3 -> {} ({})'.format(
                    np.argmax(pred1_before[0]), np.max(pred1_before[0]),
                    np.argmax(pred2_before[0]), np.max(pred2_before[0]),
                    np.argmax(pred3_before[0]), np.max(pred3_before[0])
                ) + bcolors.ENDC)
            ...
            print(bcolors.OKBLUE + 'Models predictions after: '
                ...
                'Model 3 -> {} (Conf: {:.4f}), (Original predicion conf: {:.4f})'.format(
                    label1, np.max(pred1[0]), pred1[0][orig_label],  
                    label2, np.max(pred2[0]), pred2[0][orig_label],  
                    label3, np.max(pred3[0]), pred3[0][orig_label]  
                ) + bcolors.ENDC)
            ...
            imsave('./generated_inputs/' + args.transformation + '_' + str(predictions1) + '_' + str(
                predictions2) + '_' + str(predictions3) + '_' + str(i) + '.png',
                   gen_img_deprocessed)
            imsave('./generated_inputs/' + args.transformation + '_' + str(predictions1) + '_' + str(
                predictions2) + '_' + str(predictions3) + '_' + str(i) + '_orig.png',
                   orig_img_deprocessed)

            print('./generated_inputs/' + args.transformation + '_' + str(predictions1) + '_' + str(predictions2) + '_' + str(predictions3) + '_' + str(i) + '.png')
