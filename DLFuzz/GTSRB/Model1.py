...
def Model1(input_tensor=None, train=False, retrain = False, newData = None):
    nb_classes = 43
    ...
    if train or retrain:
        ...
        if newData is not None:
            new_train_df = newData
            df_train = pd.concat([train_df, new_train_df], axis=0, ignore_index=True)
            x_train = np.array([load_and_preprocess_image(path, (img_rows, img_cols)) for path in df_train['Path'] + train_df['Path']])
    ...
    if train:
        ...
        model.load_weights(os.path.join(script_dir, "Model1.h5"))
        ...
        model.save_weights('./Model1_retrained.h5')
