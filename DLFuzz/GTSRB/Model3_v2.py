'''
Model 3 Versió 2:Utilitzant optimitzador Adam
'''

# usage: python Model3_v2.py - train the model
...
def Model3(input_tensor=None, train=False, retrain = False, newData = None, newTestData = None, inference_retrain = None):
    ...
    if train:
        ...
        # save model
        model.save_weights('./Model3_v2.h5')
        ...
        model.load_weights(os.path.join(script_dir, "Model3_v2.h5"))
        print(bcolors.OKBLUE + 'Model3 V2 weights loaded for retraining' + bcolors.ENDC)
        ...
        model.save_weights('./Model3_v2_retrained.h5')
        print(bcolors.OKGREEN + 'Model3 V2 retrained and weights saved' + bcolors.ENDC)
        
        
    else:
        if inference_retrain is None:
          model.load_weights(os.path.join(script_dir, "Model3_v2.h5"))
          print(bcolors.OKBLUE + 'Model3 V2 loaded' + bcolors.ENDC)
        else:
          model.load_weights(os.path.join(script_dir, "Model3_v2_retrained.h5"))
          print(bcolors.OKBLUE + 'Model3 V2 retrained loaded' + bcolors.ENDC)
