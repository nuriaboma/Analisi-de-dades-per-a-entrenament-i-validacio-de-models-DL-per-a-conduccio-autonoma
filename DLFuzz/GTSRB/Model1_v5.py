'''
Model 1 Versió 5: Modificant optimitzador i sense capes BatchNormalization
'''


# usage: python Model1_v5.py - train the model
...
def Model1(input_tensor=None, train=False, retrain = False, newData = None, newTestData = None, inference_retrain = None):
    ...
    if train:
        ...
        # save model
        model.save_weights('./Model1_v5.h5')
        ...
        model.load_weights(os.path.join(script_dir, "Model1_v5.h5"))
        print(bcolors.OKBLUE + 'Model V5 weights loaded for retraining' + bcolors.ENDC)
        ...
        model.save_weights('./Model1_v5_retrained.h5')
        print(bcolors.OKGREEN + 'Model V4 retrained and weights saved' + bcolors.ENDC)

    else:
        if inference_retrain is None:
          model.load_weights(os.path.join(script_dir, "Model1_v5.h5"))
          print(bcolors.OKBLUE + 'Model1 V5 loaded' + bcolors.ENDC)
        else:
          model.load_weights(os.path.join(script_dir, "Model1_v5_retrained.h5"))
          print(bcolors.OKBLUE + 'Model1 V5 retrained loaded' + bcolors.ENDC)
