...
# img_dir = '/content/seeds_per_DLFuzz'
...
for i in xrange(img_num):
    ...
    tmp_img = load_and_preprocess_image(img_path, (img_rows, img_cols))
    ...
    while len(img_list) > 0:
        ...
        for iters in xrange(iteration_times):
            ...
            if label1 != orig_label:
                print(bcolors.OKBLUE + 'Models predictions before: {} (Conf: {:.4f})'.format(
                    decode_label(np.argmax(pred1_before[0])), np.max(pred1_before[0])
                    ) + bcolors.ENDC)
                print(bcolors.OKBLUE + 'Models predictions after: {} (Conf: {:.4f}), (Original predicion conf: {:.4f}),'.format(
                    decode_label(np.argmax(pred1[0])), np.max(pred1[0]), pred1[0][orig_label]
                    ) + bcolors.ENDC)

                get_top_predictions(pred1_before, pred1, model_name[-1])
                
                print(bcolors.DARKYELLOW + 'EMD Score: Model {} -> {}'.format(
                    model_name[-1], wasserstein_distance(range(len(pred1_before[0])), range(len(pred1[0])), pred1_before[0], pred1[0])
                    ) + bcolors.ENDC)
                ...
                print('generated image: ', save_img)
