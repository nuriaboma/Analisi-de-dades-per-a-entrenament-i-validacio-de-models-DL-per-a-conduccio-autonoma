'''
Model 1 Versió 3: Modificant neurones de les capes
'''


# usage: python Model1_v3.py - train the model
...
def Model1(input_tensor=None, train=False, retrain = False, newData = None, newTestData = None, inference_retrain = None):
    ...
    # Block 1
    x = Conv2D(32, kernel_size=(3,3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(l2_lambda))(input_tensor)
    x = Conv2D(64, kernel_size=(3,3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(l2_lambda))(x)
    ...
    if train:
        ...
        # save model
        model.save_weights('./Model1_v3.h5')
        ...
        model.load_weights(os.path.join(script_dir, "Model1_v3.h5"))
        ...
        model.save_weights('./Model1_v3_retrained.h5')
        print(bcolors.OKGREEN + 'Model retrained and weights saved' + bcolors.ENDC)

    else:
        if inference_retrain is None:
          model.load_weights(os.path.join(script_dir, "Model1_v3.h5"))
          print(bcolors.OKBLUE + 'Model1 V3 loaded' + bcolors.ENDC)
        else:
          model.load_weights(os.path.join(script_dir, "Model1_v3_retrained.h5"))
          print(bcolors.OKBLUE + 'Model1 V3 retrained loaded' + bcolors.ENDC)
