...
from utils_tmp_imagenet import *
import sys
import os
import time
...
for i in xrange(img_num):
    ...
    while len(img_list) > 0:
        ...
        print(bcolors.OKBLUE + 'Models predictions before: {} ({})'.format(
              decode_label(pred1), np.max(pred1[0])
              ) + bcolors.ENDC)
        ...
        for iters in xrange(iteration_times):
            ...
            if label1 != orig_label:
                print(bcolors.OKBLUE + 'Models predictions after: {} ({})'.format(
                    decode_label(pred1), np.max(pred1[0])
                    ) + bcolors.ENDC)
                ...
                print(bcolors.DARKYELLOW + 'EMD Score: Model {} -> {}'.format(
                    model_name[-5:], wasserstein_distance(range(len(pred1_before[0])), range(len(pred1[0])), pred1_before[0], pred1[0])
                    ) + bcolors.ENDC)
