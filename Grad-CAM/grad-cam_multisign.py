...
def grad_cam(input_model, image, category_index, layer_name):
    ...
    cam = cv2.resize(cam, (image.shape[2], image.shape[1]))  # (W, H)
...
if not os.path.exists(os.path.join('/content/Grad-CAM_Signs_Other', sys.argv[3])):
    os.makedirs(os.path.join('/content/Grad-CAM_Signs_Other', sys.argv[3]))

filename = os.path.basename(image_path)

pattern = r"^(.*?)(_?(?:jpg|jpeg|png))?\.rf"
...
output_path = '/content/Grad-CAM_Signs_Other/' + sys.argv[3] + '/gradcam__' + basename + '__@' + decode_predictions(np.argmax(predictions[0])) + '@__' + str(conf) + '__' + model_name + '.jpg'
output_path_guided = '/content/Grad-CAM_Signs_Other/' + sys.argv[3] + '/guided_gradcam__' + basename + '__@' + decode_predictions(np.argmax(predictions[0])) + '@__' + str(conf) + '__' + model_name + '.jpg'
